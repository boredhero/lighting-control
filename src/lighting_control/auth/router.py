...
async def list_invites(admin: User = Depends(require_admin), db: AsyncSession = Depends(get_session)):
    return await service.get_active_invites(db)
...
async def revoke_invite(invite_id: str, admin: User = Depends(require_admin), db: AsyncSession = Depends(get_session)):
    if not await service.revoke_invite(db, invite_id):
